...
//Returns: whether any particles are still active
{
	...
	for (UINT wIndex=wParticleCount; wIndex--; )
	{
...
//O-square obstacle?
{
	...
	const UINT wOTileNo = pRoom->GetOSquare(
		((Sint16)particle.x - this->screenRect.x) / CBitmapManager::CX_TILE,
		((Sint16)particle.y - this->screenRect.y) / CBitmapManager::CY_TILE);
	switch (wOTileNo)
	{
