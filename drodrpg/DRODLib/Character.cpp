...
#define STOP_COMMAND {if (!this->wJumpLabel) goto Finish; bIfConditionFailed = true; bProcessNextCommand=true; break;}
...
//Returns: whether the specified game element (flags) is in rect (x,y,w,h).
{
	...
	for (UINT y=py; y <= endY; ++y)
	{
		...
		{
			if (room.DoesSquareContainTile(x, y, tile))
				return true;
		}
	}
