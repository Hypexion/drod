...
const UINT TAG_MINIMAP_ICON_POSITION_LISTBOX = 883;
...
//Params:
	...
	, pMinimapIconPositionListBox(NULL), pMinimapIconListBox(NULL)
...
//Separated from constructor for readability.
{
	...
	static const int Y_WAITFLAGSLISTBOX = Y_ONOFFLISTBOX + CY_ONOFFLISTBOX + CY_LABEL;
	static const UINT CX_WAITFLAGSLISTBOX = 100;
	static const UINT CY_WAITFLAGSLISTBOX = 5* LIST_LINE_HEIGHT + 4;

	static const int X_IGNOREFLAGSLABEL = X_WAITFLAGSLISTBOX;
	static const int Y_IGNOREFLAGSLABEL = Y_WAITFLAGSLISTBOX - CY_LABEL;
	...
	//Minimap icons
	static const int X_MINIMAP_ICON_LOCATION_LISTBOX = X_DIRECTIONLISTBOX;
	static const int Y_MINIMAP_ICON_LOCATION_LISTBOX = Y_DIRECTIONLISTBOX;
	static const int CX_MINIMAP_ICON_LOCATION_LISTBOX = CX_DIRECTIONLISTBOX;
	static const int CY_MINIMAP_ICON_LOCATION_LISTBOX = 9 * LIST_LINE_HEIGHT + 4; //9 entries;

	static const int X_MINIMAP_ICON_LISTBOX = X_MINIMAP_ICON_LOCATION_LISTBOX +
		CX_MINIMAP_ICON_LOCATION_LISTBOX + CX_SPACE;
	static const int Y_MINIMAP_ICON_LISTBOX = Y_MINIMAP_ICON_LOCATION_LISTBOX;
	static const int CX_MINIMAP_ICON_LISTBOX = CX_DIRECTIONLISTBOX;
	static const int CY_MINIMAP_ICON_LISTBOX = 9 * LIST_LINE_HEIGHT + 4; //? entries;
...
//	this->pWaitFlagsListBox->AddItem(ScriptFlag::BEETHRO, g_pTheDB->GetMessageText(MID_Beethro));
	...
	this->pMovementTypeListBox = new CListBoxWidget(TAG_MOVETYPELISTBOX,
		X_ONOFFLISTBOX2, Y_IMPERATIVELISTBOX, CX_MOVETYPELISTBOX, CY_MOVETYPELISTBOX);
	...
	this->pAddCommandDialog->AddWidget(new CLabelWidget(TAG_IGNOREFLAGS_LABEL, X_IGNOREFLAGSLABEL, Y_IGNOREFLAGSLABEL,
	...
	//minimap icons
	this->pMinimapIconPositionListBox = new CListBoxWidget(TAG_MINIMAP_ICON_POSITION_LISTBOX,
		X_MINIMAP_ICON_LOCATION_LISTBOX, Y_MINIMAP_ICON_LOCATION_LISTBOX,
		CX_MINIMAP_ICON_LOCATION_LISTBOX, CY_MINIMAP_ICON_LOCATION_LISTBOX);
	this->pAddCommandDialog->AddWidget(this->pMinimapIconPositionListBox);
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_NorthWest, g_pTheDB->GetMessageText(MID_NorthWest));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_North, g_pTheDB->GetMessageText(MID_North));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_NorthEast, g_pTheDB->GetMessageText(MID_NorthEast));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_West, g_pTheDB->GetMessageText(MID_West));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_Center, g_pTheDB->GetMessageText(MID_Center));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_East, g_pTheDB->GetMessageText(MID_East));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_SouthWest, g_pTheDB->GetMessageText(MID_SouthWest));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_South, g_pTheDB->GetMessageText(MID_South));
	this->pMinimapIconPositionListBox->AddItem(ScriptVars::MIP_SouthEast, g_pTheDB->GetMessageText(MID_SouthEast));
...
const
{
	...
	switch (command.command)
	{
		...
		case CCharacterCommand::CC_SetMinimapIcon:
			...
			wstr += wszRightParen;
			wstr += wszSpace;
			wstr += this->pMinimapIconPositionListBox->GetTextForKey(command.w);
			wstr += wszSpace;
			wstr += this->pMinimapIconListBox->GetTextForKey(command.h);
		break;
...
void CCharacterDialogWidget::PopulateItemGroupListBox(CListBoxWidget* pListBox)
{
	...
	pListBox->AddItem(ScriptFlag::IG_Trapdoor, g_pTheDB->GetMessageText(MID_TrapdoorGroup));
	...
	pListBox->AddItem(ScriptFlag::IG_MoneyDoor, g_pTheDB->GetMessageText(MID_MoneyDoor));
	pListBox->AddItem(ScriptFlag::IG_DirtBlock, g_pTheDB->GetMessageText(MID_Dirt1));
...
//Enable those widgets that are applicable to currently selected command.
{
	...
	static const UINT widgetTag[NUM_WIDGETS] = {
		...
		TAG_MINIMAP_ICON_POSITION_LISTBOX, TAG_MINIMAP_ICON_LISTBOX
	};
	...
	static const UINT MINIMAPICON[] = { TAG_MINIMAP_ICON_POSITION_LISTBOX, TAG_MINIMAP_ICON_LISTBOX, 0 };
	...
	CListBoxWidget *pActiveCommandList, COMMANDPTR_VECTOR& commands)
{
	...
	switch (this->pCommand->command)
	{
		...
		break;
		case	CCharacterCommand::CC_SetMinimapIcon:
			this->pCommand->w = this->pMinimapIconPositionListBox->GetSelectedItem();
			this->pCommand->h = this->pMinimapIconListBox->GetSelectedItem();
			QueryMapRoom();
		break;
...
//Set state of command settings widgets from command state.
{
	...
	switch (this->pCommand->command)
	{
		...
		break;
		case CCharacterCommand::CC_SetMinimapIcon:
			this->pMinimapIconPositionListBox->SelectItem(this->pCommand->w);
			this->pMinimapIconListBox->SelectItem(this->pCommand->h);
		break;
